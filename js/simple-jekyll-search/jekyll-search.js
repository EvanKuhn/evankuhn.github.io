  ...
  // for backwards compatibility
      ...
      function registerInput () {
        options.searchInput.addEventListener('keyup', function (e) {
          ...
          // Clear results list if query is empty
          if (query.length == 0) {
            emptyResultsContainer()
          }
          // Clear the text field and results list if Esc key is pressed
          else if (key === 27) {
            emptyResultsContainer()
            e.target.value = ''
          }
          // Otherwise, search for results and list them
          else if (isWhitelistedKey(key) && isValidQuery(query)) {
            emptyResultsContainer()
